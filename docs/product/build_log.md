...
|------|-------------|--------|------------|-----------|--------------|----------------------|-----------------|--------|
| 2026-03-03 | **Hope V6.0 drafted — PENDING DEPLOY.** Path architecture (A–G) replacing MODE/URGENCY model. KB-only URL enforcement (removed all hardcoded URLs from prompt). Three-tier escalation model (Tier 2 scheduling offer before Tier 3 ASAP fallback). Consultation booking flow added. Private tour path overhauled. Personality section consolidated into prompt. V5.0 remains live until 23/23 pass + Todd sign-off. | Cleaner intent routing, KB-only URL compliance, improved private tour and consultation handling. | Med | ❌ Not yet run — 23/23 required before deploy | Voice AI Quality Score | Y — rollback = restore V5.0 | N — required before deploy | 🟡 DRAFT. Awaiting regression test run. |
